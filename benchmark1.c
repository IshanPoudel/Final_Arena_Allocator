...
/* Helper functions for //printfing*/
void printfNode(int index)
{
    printf("\n");
...
int insertNodeInternal(int previous_index , int current_index , int size )
{
    ...
    if (LinkedList[rootNode].previous==-1 && LinkedList[rootNode].next==-1)
    {
        ...
        ////printff("I am adding when everything is empty. Need to initialize with a hole\n");
        ...
        // ////printff("The next free is at %d\n" , next_free);

        ////printff("I am returning the index %d where i added %d when I added to an empty head\n" , rootNode , size );
        return rootNode;
    ...
    else if (previous_index>=0)
    {
        ...
        ////printff("I am here when previous_index >= 0  . The value of previous_index is %d. \n\n" , previous_index);
        ...
        {
            ////printff("I found the perfect hole\n\n");
            ...
            ////printff("I am returning the index %d where i added %d when I added an in between in a perfect hole\n" , current_index , size );
        ...
        else
        {
            ...
            ////printff("The index of the hole is %d\n" , index_of_hole);
            
            

            LinkedList[current_index].previous=previous_index;
            ////printff("Set LinkedList[%d].previous_index to %d\n" , current_index , previous_index);

            LinkedList[previous_index].next=current_index;
            ////printff("Set LinkedList[%d].next to %d\n" , previous_index , current_index);
            ...
            // ////printff("Added Linkedlist[%d].previous_index is Linkedlist[%d]\n" , current_index , LinkedList[previous_index].previous_index);
            ...
            ////printff("I am returning the index %d where i added %d and the remaining was a hole\n" , current_index , size );
            return current_index;
    ...
    else 
    {
        ...
        ////printff("I am adding to the front of the head , but there are processes behind it.\n");
        ////printff("The current_index index is %d , the previous_index index is %d \n" , current_index , previous_index);
        ...
        if (LinkedList[rootNode].size==size)
        {
            ////printff("We have a perfect fit at the front of the head\n");
            LinkedList[rootNode].type=PROCESS;
            ////printff("i am adding to front of the head but processes behind and it is a perferct fit. I returned %d\n" , rootNode);
            return rootNode;
        }
        else
        {
            ...
            ////printff("i am adding to front of the head but processes behind and created a hole. I returned %d\n" , rootNode);
            return rootNode;
...
int insertNode_FirstFit(size_t size)
{
    ...
    current = rootNode;
    //////printff("The current root is at index %d\n" , current);
    int previous = -1;
    int ret=-1;
    ...
    {
        //////printff("There is a free node at the head.\n");
        previous=-1 ;
    ...
    else
    {
        while( current>=0 && LinkedList[current].in_use )
        {
            ...
            if(LinkedList[LinkedList[current].next].size>=size && LinkedList[LinkedList[current].next].type==HOLE)
            {
                ...
                //////printff("MATCH: Current node at index %d is a %s and has size %zu\n" , current ,enum_string[LinkedList[current].type] , LinkedList[current].size);
                //////printff("MATCH: Next index is a hole\n");
                previous=current;
                break;
    ...
    //////printff("The previous value %d  is currently filled with a process and the next one is either not in use or a hole . \n" , previous );
    if (previous>=0)
    {
         //printfNode(previous);
    ...
    // //printfNode(index);
    // //printfNode(LinkedList[index].next);
...
int removeNodeInternal(int node)
{
  ...
  {
    ////printff("Cannot remove node. Not in use\n");
    return -1;
  }
...
void mavalloc_free( void * ptr )
{
  ...
  while (index!=-1)
  {
    ...
    //   ////printff("I removed node at index %d  whose pointer is supposed to be %p\n" , index , ptr);
